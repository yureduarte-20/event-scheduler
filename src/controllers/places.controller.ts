...
import { IPlace, IUserPlaces, Place, PLACE_TABLE } from "../model/place.model";
...
class PlacesController {
    ...
    async index(request: Request, response: Response) {
        ...
        return response.json({
            places: await connection(PLACE_TABLE).select('*').limit(PlacesController.count_page).offset( (page -1) * PlacesController.count_page ),
            nextPage: page + 1,
        })
    }
    ...
    async edit(request: AutenticatedRequest, response: Response) {
        ...
        const { cep, city, country, name, state, neighborhood, street }: Partial<IPlace> = request.body;
