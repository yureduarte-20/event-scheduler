...
class PlacesController {
    ...
    async create(request: AutenticatedRequest, response: Response) {
        const { city, country, name, state, address_number,  neighborhood, street  }: IPlace = request.body
        const { userId } = request;
        try {
            const id: any = await Place.create({ city, country, name, state, address_number,  neighborhood, street });
            await connection.insert<IUserPlaces>({ user_id: userId, place_id: id[0] }).into("user_places")
    ...
    async edit(request: AutenticatedRequest, response: Response) {
        ...
        const { city, country, name, state, neighborhood, street }: Partial<IPlaceModel> = request.body;
    ...
    async find(request: Request, response: Response) {
        const place = await Place.findById(parseInt(request.params.id));
        if(!place ) return response.status(404).json({ error:{ message:"lugar não encontrado" } })
